...
This fix contains an important bug-fix for a regression I introduced into the
`prefetch` function that stripped grouping and having clauses from subqueries.
I'm not sure why that was added, probably an attempt to optimize one of the
use-cases, but it unfortunately broke other people's code. See #1077
