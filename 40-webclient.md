...
## Этап 2. Создание приложения Node.js.
...
## Этап 3. Подключение сервиса Watson IOT.
...
## Этап 4. Принципы обмена MQTT-сообщениями в Bluemix и создание MQTT Device и MQTT API key.
...
## Этап 5. Описание подключения к MQTT брокеру и проверка передачи MQTT сообщений с помощью MQTTlens.
...
## Этап 7. Подключение сервиса Compose for MongoDB, краткое описание MongoDB и MongoShell, описание примера проекта для хакатона.
...
### Описание примера проекта.

Тестовый проект включает в себя вебсервер на Node.js, который взаимодействует с сервисами Watson IOT и Compose for MongoDB. По ссылке https://mqttlisteneriu.mybluemix.net/showChart показывает график, который отображает последние 10 хранимых в базе данных значений.
...
mongoose.js – файл с описанием схемы для модели mongoose (подробнее  читать https://mongoosejs.com/)  
