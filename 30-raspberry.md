...
Для реализации отдачи HTML страницы с актуальными данными был выбран фреймворк [Flask](https://flask.pocoo.org/), с помощью которого можно постпроить серверное приложение в короткие сроки. Для работы с базой данных используется ORM ( модель объектно-реляционного отображения данных ) [peewee](https://docs.peewee-orm.com/en/latest/peewee/quickstart.html), которая позволяет работать с сущностями БД как с объектами.
...
Код в index.html: ([Пример работы с HTML кодом](https://www.w3schools.com/html/))
```
<!DOCTYPE html>
<html lang="en">
<head>
...
Для запуска проекта необходимо развернуть [виртуальное окружение](https://docs.python-guide.org/en/latest/dev/virtualenvs/) и установить нужные зависимости:
